...
async def async_stream_generator(client: httpx.AsyncClient, response: httpx.Response) -> AsyncGenerator[bytes, None]:
    try:
        async for chunk in response.aiter_bytes(chunk_size=65536): # 64KB chunks for smooth buffering
            yield chunk
    finally:
...
async def stream_video(file_id: str, request: Request):
    ...
    Meneruskan request range ke Google Drive API dan mengembalikan Status 206 Partial Content.
    """
    try:
    ...
    try:
        ...
        # Siapkan header balasan untuk browser
        response_headers = {
            "Accept-Ranges": "bytes",
...
async def preview_image(file_id: str):
    ...
    try:
        ...
        response_headers = {
            ...
            "Cache-Control": "public, max-age=86400", # Cache di browser 24 jam
        }
        if "Content-Length" in res.headers:
            response_headers["Content-Length"] = res.headers["Content-Length"]
...
async def download_file(file_id: str, filename: str = "file"):
    """
